...
object MaintenanceManager {
  ...
  private def handleCommand(context: ActorContext[Command])(state: State[ManagerState], command: Command): Effect[Event, State[ManagerState]] =
    command match {
          ...
          .thenRun(_ => {
            ...
            if (maintenanceActors.isEmpty) {
              ...
              val futures = maintenanceActors.map { case (id, actor) =>
                actor.ask[CommandResponse[Maintenance]](ref => Command.Get(id, ref))(timeout, context.system.scheduler)
                  .map(response => id -> response)
              }

              Future.sequence(futures).foreach { responses =>
                val result = responses.collect {
                  case (id, CommandResponse.Success(Some(maintenance))) => id -> maintenance
                }.toMap
      ...
      case createMaintenance@Command.Create(maintenance, _) =>
        val ref = context.spawn(Maintenance(maintenance.id), s"maintenance-${maintenance.id}")
  ...
  private def handleEvent(context: ActorContext[Command])(state: State[ManagerState], event: Event): State[ManagerState] =
    event match {
      case Created(maintenance) =>
        val maintenanceActorId = s"maintenance-${maintenance.id}"
        val maintenanceActorRef = context.child(maintenanceActorId) // exists after command handler,
          .getOrElse(context.spawn(Maintenance(maintenance.id), maintenanceActorId))
          .unsafeUpcast[AnyRef]
          .narrow[Command]
        val maintenanceActors = state.state.getOrElse(ManagerState(Map.empty)).actors
